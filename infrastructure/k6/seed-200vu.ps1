# ============================================================================
# HMS Backend - 200 VU Complete Seed Script (API-based)
# ============================================================================
# Creates accounts and essential records for 200 VU load testing
#
# Accounts (200 total):
#   - 5 Admin (admin1-5@hms.com) - Admin123!@
#   - 40 Doctor (doctor1-40@hms.com) - Doctor123!@
#   - 30 Nurse (nurse1-30@hms.com) - Nurse123!@
#   - 25 Receptionist (receptionist1-25@hms.com) - Receptionist123!@
#   - 120 Patient (patient1-120@email.com) - Patient123!@
#
# Essential Records:
#   - 6 Departments
...
#   - Some initial appointments
#
# Usage: .\seed-200vu.ps1 [-BaseUrl "http://localhost:8080"]
# ============================================================================
...
# Wait for services
# ============================================================================
Write-Header "HMS Backend - 200 VU Complete Seed Script"
...
# ============================================================================
# Step 2: Create Doctor Accounts & Employees (40)
# ============================================================================
Write-Header "Step 2: Create Doctor Accounts & Employees (40)"
...
for ($i = 1; $i -le 40; $i++) {
    ...
    if ($i % 10 -eq 0) { Write-Ok "Created $i doctors" }
}
Write-Info "Doctor employees: $($script:DoctorEmployeeIds.Count)"

# ============================================================================
# Step 3: Create Nurse Accounts & Employees (30)
# ============================================================================
Write-Header "Step 3: Create Nurse Accounts & Employees (30)"
...
for ($i = 1; $i -le 30; $i++) {
    ...
    if ($i % 10 -eq 0) { Write-Ok "Created $i nurses" }
}

# ============================================================================
# Step 4: Create Receptionist Accounts & Employees (25)
# ============================================================================
Write-Header "Step 4: Create Receptionist Accounts & Employees (25)"
...
for ($i = 1; $i -le 25; $i++) {
    ...
    if ($i % 10 -eq 0) { Write-Ok "Created $i receptionists" }
}
Write-Ok "Created 25 receptionists"

# ============================================================================
# Step 5: Create Admin Accounts (5)
# ============================================================================
Write-Header "Step 5: Create Admin Accounts (5)"
...
if ($script:DoctorEmployeeIds.Count -lt 40) {
    Write-Info "Fetching existing doctor employees..."
    $response = Invoke-Api -Method GET -Endpoint "/api/hr/employees/all?filter=role==DOCTOR&page=0&size=50" -Token $script:AdminToken
    if ($response -and $response.data -and $response.data.content) {
        $idx = 1
...
# ============================================================================
# Step 7: Create Patient Accounts & Profiles (120)
# ============================================================================
Write-Header "Step 7: Create Patient Accounts & Profiles (120)"
...
for ($i = 1; $i -le 120; $i++) {
    ...
    if ($i % 20 -eq 0) { Write-Ok "Created $i patients" }
}
...
if ($categoryId) {
    $medicines = @(
        @{ name = "Aspirin 100mg"; activeIngredient = "Aspirin"; unit = "Tablet"; quantity = 5000; purchasePrice = 10000; sellingPrice = 15000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "PharmaCorp"; categoryId = $categoryId },
        @{ name = "Lisinopril 10mg"; activeIngredient = "Lisinopril"; unit = "Tablet"; quantity = 3000; purchasePrice = 50000; sellingPrice = 65000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "CardioPharm"; categoryId = $categoryId },
        @{ name = "Amoxicillin 500mg"; activeIngredient = "Amoxicillin"; unit = "Capsule"; quantity = 4000; purchasePrice = 30000; sellingPrice = 45000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "MediCo"; categoryId = $categoryId },
        @{ name = "Paracetamol 500mg"; activeIngredient = "Paracetamol"; unit = "Tablet"; quantity = 10000; purchasePrice = 5000; sellingPrice = 8000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "GenericPharma"; categoryId = $categoryId },
        @{ name = "Ibuprofen 400mg"; activeIngredient = "Ibuprofen"; unit = "Tablet"; quantity = 6000; purchasePrice = 15000; sellingPrice = 22000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "PainRelief Inc"; categoryId = $categoryId },
        @{ name = "Metformin 500mg"; activeIngredient = "Metformin"; unit = "Tablet"; quantity = 4000; purchasePrice = 20000; sellingPrice = 30000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "DiabCare"; categoryId = $categoryId },
        @{ name = "Omeprazole 20mg"; activeIngredient = "Omeprazole"; unit = "Capsule"; quantity = 3000; purchasePrice = 25000; sellingPrice = 35000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "GastroMed"; categoryId = $categoryId },
        @{ name = "Vitamin C 1000mg"; activeIngredient = "Ascorbic Acid"; unit = "Tablet"; quantity = 8000; purchasePrice = 8000; sellingPrice = 12000; expiresAt = "2027-12-31T00:00:00Z"; manufacturer = "VitaCorp"; categoryId = $categoryId }
    )
...
$testAccounts = @(
    ...
    @{ email = "doctor40@hms.com"; password = "Doctor123!@"; role = "DOCTOR" },
    ...
    @{ email = "patient120@email.com"; password = "Patient123!@"; role = "PATIENT" }
)
...
Write-Host "  - Doctor employees: ~40"
Write-Host "  - Nurse employees: ~30"
Write-Host "  - Receptionist employees: ~25"
Write-Host "  - Doctor schedules: ~280 (40 doctors x 7 days)"
...
Write-Host ""
Write-Host "Accounts (220 total):" -ForegroundColor Yellow
Write-Host "  - 5 Admin (admin1-5@hms.com) - Admin123!@"
Write-Host "  - 40 Doctor (doctor1-40@hms.com) - Doctor123!@"
Write-Host "  - 30 Nurse (nurse1-30@hms.com) - Nurse123!@"
Write-Host "  - 25 Receptionist (receptionist1-25@hms.com) - Receptionist123!@"
Write-Host "  - 120 Patient (patient1-120@email.com) - Patient123!@"
Write-Host ""

if ($allPassed) {
    Write-Host "All login tests PASSED! Ready for k6 load testing with 200 VUs." -ForegroundColor Green
} else {
