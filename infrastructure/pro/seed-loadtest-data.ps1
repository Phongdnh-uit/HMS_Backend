...
function Execute-SQL {
    ...
    # Write SQL to a temp file on host, copy to container, execute, cleanup
    $tempHostFile = [System.IO.Path]::GetTempFileName()
    $tempContainerFile = "/tmp/seed_sql.sql"
    
    # Write SQL to host temp file
    Set-Content -Path $tempHostFile -Value $SQL -Encoding UTF8
    
    # Copy to container (redirect ALL output to suppress "Successfully copied" message)
    $null = docker cp $tempHostFile "${Container}:${tempContainerFile}" 2>&1
    
    # Execute the SQL file
    $ErrorActionPreference = 'SilentlyContinue'
    $result = docker exec $Container mysql -u $User -psecret $Database -e "source $tempContainerFile" 2>&1 | Where-Object { $_ -notmatch '\[Warning\]' }
    $ErrorActionPreference = 'Continue'
    
    # Cleanup
    $null = docker exec $Container rm -f $tempContainerFile 2>&1
    Remove-Item $tempHostFile -Force -ErrorAction SilentlyContinue
...
# Password hashes (BCrypt) for each role - extracted from working AdminSeeder accounts:
# Admin: Admin123!@ | Doctor: Doctor123!@ | Nurse: Nurse123!@ | Receptionist: Receptionist123!@ | Patient: Patient123!@

# Note: Using individual inserts per account (like admin) since batched inserts have issues

# Admins (5) - Password: Admin123!@
$adminHash = '$2a$10$viLcx/1Ae8fRLs26p4qoWubvRgP5LDZhB8XsQzZkqkQyBXi.Anyn6'
for ($i = 1; $i -le 5; $i++) {
    $authSQL = "INSERT IGNORE INTO accounts (id, email, password, role, email_verified) VALUES (UUID_TO_BIN(CONCAT(SUBSTR(MD5(CONCAT('admin-', $i)), 1, 8), '-', SUBSTR(MD5(CONCAT('admin-', $i)), 9, 4), '-', SUBSTR(MD5(CONCAT('admin-', $i)), 13, 4), '-', SUBSTR(MD5(CONCAT('admin-', $i)), 17, 4), '-', SUBSTR(MD5(CONCAT('admin-', $i)), 21, 12))), 'admin${i}@hms.com', '" + $adminHash + "', 'ADMIN', true);"
...
# Doctors (60) - Password: Doctor123!@
$doctorHash = '$2a$10$Gwhi/Kt0chDuBIDM2qdYJun9Xp.wwa4zOfn7CtJ6KPtIzPiart4Ku'
for ($i = 1; $i -le 60; $i++) {
    $authSQL = "INSERT IGNORE INTO accounts (id, email, password, role, email_verified) VALUES (UUID_TO_BIN(CONCAT(SUBSTR(MD5(CONCAT('doctor-', $i)), 1, 8), '-', SUBSTR(MD5(CONCAT('doctor-', $i)), 9, 4), '-', SUBSTR(MD5(CONCAT('doctor-', $i)), 13, 4), '-', SUBSTR(MD5(CONCAT('doctor-', $i)), 17, 4), '-', SUBSTR(MD5(CONCAT('doctor-', $i)), 21, 12))), 'doctor${i}@hms.com', '" + $doctorHash + "', 'DOCTOR', true);"
    Execute-SQL -Container $SERVICES["auth-service"].Container -Database $SERVICES["auth-service"].Database -User $SERVICES["auth-service"].User -SQL $authSQL
}
# Nurses (50) - Password: Nurse123!@
$nurseHash = '$2a$10$7/7I.J68344Dv03Q/ZtrIOyLDZpyYHciHBWsVQX6B1RCkRDRmoXzG'
for ($i = 1; $i -le 50; $i++) {
    $authSQL = "INSERT IGNORE INTO accounts (id, email, password, role, email_verified) VALUES (UUID_TO_BIN(CONCAT(SUBSTR(MD5(CONCAT('nurse-', $i)), 1, 8), '-', SUBSTR(MD5(CONCAT('nurse-', $i)), 9, 4), '-', SUBSTR(MD5(CONCAT('nurse-', $i)), 13, 4), '-', SUBSTR(MD5(CONCAT('nurse-', $i)), 17, 4), '-', SUBSTR(MD5(CONCAT('nurse-', $i)), 21, 12))), 'nurse${i}@hms.com', '" + $nurseHash + "', 'NURSE', true);"
    Execute-SQL -Container $SERVICES["auth-service"].Container -Database $SERVICES["auth-service"].Database -User $SERVICES["auth-service"].User -SQL $authSQL
}

# Receptionists (40) - Password: Receptionist123!@
$receptionistHash = '$2a$10$WilJNar/8NPJKKWHdsOenOA/avfi3qiFBI1THDaU80zGIiVVFlgOS'
for ($i = 1; $i -le 40; $i++) {
    $authSQL = "INSERT IGNORE INTO accounts (id, email, password, role, email_verified) VALUES (UUID_TO_BIN(CONCAT(SUBSTR(MD5(CONCAT('receptionist-', $i)), 1, 8), '-', SUBSTR(MD5(CONCAT('receptionist-', $i)), 9, 4), '-', SUBSTR(MD5(CONCAT('receptionist-', $i)), 13, 4), '-', SUBSTR(MD5(CONCAT('receptionist-', $i)), 17, 4), '-', SUBSTR(MD5(CONCAT('receptionist-', $i)), 21, 12))), 'receptionist${i}@hms.com', '" + $receptionistHash + "', 'RECEPTIONIST', true);"
    Execute-SQL -Container $SERVICES["auth-service"].Container -Database $SERVICES["auth-service"].Database -User $SERVICES["auth-service"].User -SQL $authSQL
}

# Patients (1000) - Password: Patient123!@
$patientHash = '$2a$10$jrsZQfH/zCFO.rHDA11oVubWtVoSi6Tzy/SQ9URxsqhw33qUWM.hC'
for ($i = 1; $i -le 1000; $i++) {
    $authSQL = "INSERT IGNORE INTO accounts (id, email, password, role, email_verified) VALUES (UUID_TO_BIN(CONCAT(SUBSTR(MD5(CONCAT('patient-', $i)), 1, 8), '-', SUBSTR(MD5(CONCAT('patient-', $i)), 9, 4), '-', SUBSTR(MD5(CONCAT('patient-', $i)), 13, 4), '-', SUBSTR(MD5(CONCAT('patient-', $i)), 17, 4), '-', SUBSTR(MD5(CONCAT('patient-', $i)), 21, 12))), 'patient${i}@email.com', '" + $patientHash + "', 'PATIENT', true);"
    Execute-SQL -Container $SERVICES["auth-service"].Container -Database $SERVICES["auth-service"].Database -User $SERVICES["auth-service"].User -SQL $authSQL
}
